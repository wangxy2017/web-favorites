...
public interface FavoritesRepository extends JpaRepository<Favorites, Integer>, JpaSpecificationExecutor<Favorites> {
    ...
    @Query("select new Favorites (f.id,f.name,f.icon,f.url,f.support,u.nickName,f.clickCount) from Favorites f left join User u on u.id = f.userId where f.isShare = 1 and f.deleteFlag = 0 and ( f.name like :name or f.pinyin like :name or f.pinyinS like :name)")
