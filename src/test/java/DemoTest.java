    ...
    public void test() {
        for (int k = 0; k < 100; k++) {
            ...
            for (int i = 0; i < 100; i++) {
                ...
                for (int j = 0; j < 100; j++) {
                    favoritesRepository.save(new Favorites(null, "百度一下" + j, "http://www.baidu.com/favicon.ico", "http://www.baidu.com/", category.getId(), user.getId(), PinYinUtils.toPinyin("百度一下" + j), null, null, null,null));
                }
            }
        }
        log.info("批量插入数据完成！！！");
